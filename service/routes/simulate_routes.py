...
def simulate():
    ...
    print(f"NDVI Delta min: {np.nanmin(ndvi_delta)}, max: {np.nanmax(ndvi_delta)}, mean: {np.nanmean(ndvi_delta)}")
    ...
    for row in range(0, padded_rows - tile_size + 1, stride):
        for col in range(0, padded_cols - tile_size + 1, stride):
            ...
            print(f"Predicted chunk min: {np.nanmin(predicted_chunk)}, max: {np.nanmax(predicted_chunk)}, mean: {np.nanmean(predicted_chunk)}")
    ...
    new_heat_map = heat_map + heat_delta * 10
    ...
    im_new = ax_heat.imshow(heat_delta, cmap="RdBu_r", vmin=heat_vmin, vmax=heat_vmax)
    ax_heat.axis("off")
    ...
    # Return the simulated heat map with bounding box and date
    return jsonify({
        "heat_map_image": heat_map_base64,
        "bounding_box": list(bbox),
        "image_date": session_data["heat_map"]["asset_date"]
    }), 200
